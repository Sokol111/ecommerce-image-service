...
func NewJWTTokenService(secretKey string) image.TokenService {
...
type customClaims struct {
	UploadTokenClaims image.UploadTokenClaims `json:"upload"`
	jwt.RegisteredClaims
}

func (s *jwtTokenService) GenerateUploadToken(ctx context.Context, claims *image.UploadTokenClaims, ttl time.Duration) (string, error) {
	now := time.Now().UTC()
...
func (s *jwtTokenService) ValidateUploadToken(ctx context.Context, tokenString string) (*image.UploadTokenClaims, error) {
