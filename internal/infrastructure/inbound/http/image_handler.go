...
func (h *imageHandler) ConfirmUpload(ctx context.Context, req *httpapi.ConfirmRequest) (httpapi.ConfirmUploadRes, error) {
	...
	if err != nil {
		// Map errors to appropriate HTTP status codes
		switch {
		case errors.Is(err, image.ErrInvalidUploadToken),
			errors.Is(err, image.ErrImageTooLarge),
			errors.Is(err, image.ErrInvalidSize):
		...
		case errors.Is(err, image.ErrObjectNotFound):
...
func (h *imageHandler) PromoteImages(ctx context.Context, req *httpapi.PromoteRequest) (httpapi.PromoteImagesRes, error) {
	...
	if err != nil {
		// Map errors to appropriate HTTP status codes
		switch {
		case errors.Is(err, image.ErrInvalidImageOwner):
		...
		case errors.Is(err, image.ErrDraftNotFound),
...
func buildDeliveryURLResponse(result *image.GetDeliveryURLResult) *httpapi.GetDeliveryUrlOK {
